...
object AIMerchantBusinessSuggestionsService:

  private val dateFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")

  def generate(connection: Connection, username: String, merchantId: MerchantId): IO[AIMerchantBusinessSuggestionsResponse] =
    for
  ...
  private def buildPrompt(
    ...
    val lowStockText =
      ...
      else lowStockProducts.map(product => s"""{"name":"${escapeText(product.name)}","remainingStock":${product.remainingStock}}"""
      ).mkString("[", ",", "]")
    val badReviewText =
      if badReviews.isEmpty then "暂无低评分评价"
      else badReviews.map(review => s"""{"rating":${review.rating},"description":"${escapeText(review.description).take(120)}","items":"${escapeText(review.orderItemNames.mkString("、"))}"}"""
      ).mkString("[", ",", "]")
...
end AIMerchantBusinessSuggestionsService
