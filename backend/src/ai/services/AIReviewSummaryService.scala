...
import delivery.merchant.services.CatalogQueryService
...
object AIReviewSummaryService:

  def generate(connection: Connection, merchantId: MerchantId): IO[AIReviewSummaryResponse] =
    for
      merchant <- CatalogQueryService.findVisibleMerchant(connection, merchantId).flatMap {
        case Some(value) => IO.pure(value)
        case None        => IO.raiseError(HttpApiError.NotFound("未找到商家"))
      }
