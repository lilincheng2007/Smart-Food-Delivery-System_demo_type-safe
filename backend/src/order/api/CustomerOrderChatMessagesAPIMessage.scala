...
final case class CustomerOrderChatMessagesAPIMessage(orderId: OrderId, peerRole: OrderChatRole) extends APIWithRoleMessage[OrderChatMessagesResponse]:
  override def plan(connection: Connection, username: String): IO[OrderChatMessagesResponse] =
    for
      ...
      _ <- OrderChatMessageTable.markReadForPair(connection, orderId, OrderChatRole.customer, peerRole)
