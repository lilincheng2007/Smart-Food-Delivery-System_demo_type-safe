...
final case class CustomerOrderChatUnreadCountsAPIMessage() extends APIWithRoleMessage[OrderChatUnreadCountsResponse]:
  override def plan(connection: Connection, username: String): IO[OrderChatUnreadCountsResponse] =
    OrderChatUnreadService.countsForRole(connection, username, OrderChatRole.customer)
