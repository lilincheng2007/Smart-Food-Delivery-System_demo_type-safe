package delivery.media.tables.storedimage
      ...
      paths.traverse_ { path =>
        ...
        if Files.isRegularFile(path) && imageNamePattern.matcher(fileName).matches() then
          IO.blocking(Files.readAllBytes(path)).flatMap { bytes =>
            StoredImageTable.upsert(connection, StoredImage(fileName, scope, ImageUploadValidator.contentTypeForName(fileName), bytes)).void
          }
        else IO.unit
      }
    }
