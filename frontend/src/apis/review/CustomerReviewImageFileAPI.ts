...
class CustomerReviewImageFileAPI extends APIMessage<string> {
  readonly apiName = 'customerreviewimagefileapi'
...
export function uploadReviewImageFileIO(file: File): TaskIO<string> {
  return async () => {
    const fileError = getLocalImageFileError(file)
    if (fileError) throw new Error(fileError)
    const contentTypeLower = file.type.toLowerCase()
    const bytesBase64 = await fileToBase64(file)
    return sendAPI(new CustomerReviewImageFileAPI(bytesBase64, contentTypeLower, file.name || null))()
  }
}
