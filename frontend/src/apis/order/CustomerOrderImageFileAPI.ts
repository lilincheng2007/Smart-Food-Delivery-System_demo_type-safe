...
class CustomerOrderImageFileAPI extends APIMessage<string> {
  readonly apiName = 'customerorderimagefileapi'
...
export function uploadOrderImageFileIO(file: File): TaskIO<string> {
  return async () => {
    const fileError = getLocalImageFileError(file)
    if (fileError) throw new Error(fileError)
    const bytesBase64 = await fileToBase64(file)
    return sendAPI(new CustomerOrderImageFileAPI(bytesBase64, file.type.toLowerCase(), file.name || null))()
  }
}
